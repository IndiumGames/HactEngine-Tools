...
*/
    ...
    class vector {
      public:
        ...
        //iterator<random_access_iterator_tag, T, ptrdiff_t, T*, T&> begin();
		...
		%extend // this is a extra bit of SWIG code
		{
			...
			void insert_at(unsigned int idx,const T& val) throw (std::out_of_range)
			{
				if (idx>=self->size())
					throw std::out_of_range("in vector::insert()");
				self->insert(self->begin()+idx, val);
			}
		};
    };
